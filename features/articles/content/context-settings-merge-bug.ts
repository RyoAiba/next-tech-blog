...
export const contextSettingsMergeBug: Article = {
  ...
  updatedAtTimestamp: 1782054000000,
  image: "/eyecatches/context-settings-merge-bug.png",
  tags: [
...
function deepMerge&lt;T extends Record&lt;string, unknown&gt;&gt;(base: T, override: Partial&lt;T&gt;): T {
  ...
  return result;
    ...
    <p>
      今回のバグは「差分だけを保存する」という設計とシャローマージが組み合わさって起きるものです。別の解決策として、差分ではなく常に完全なSettingsオブジェクトを保存してしまう、という方法もあります。これなら保存されている値は常にすべてのキーを持っているので、マージという概念自体が不要になります。ただし設定の項目が増えるたびに保存しているデータの形を意識する必要があるので、差分保存とどちらを選ぶかは規模や設定項目の増減頻度によって変わってくるところです。
    </p>
